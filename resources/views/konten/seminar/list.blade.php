...
@section('title', 'List Seminar - Dashboard Pintaro')
@section('page-title', 'List Seminar')
@section('page-subtitle', 'Ini adalah List Seminar pelatihan Pintaro')
...
@section('konten')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <table id='tabel_seminar' width='100%' border="1" class='table table-striped' style='border-collapse: collapse;'>
                <thead>
                    <tr>
    ...
    <div class="modal fade text-left" id="add_data_seminar" tabindex="-1" role="dialog" aria-labelledby="modal_add"
        aria-hidden="true">
        <div class="modal-dialog modal-create2 modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="modal_add">Tambah Seminar</h4>
                ...
                <form>
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <input type="text" placeholder="tempat seminar" class="form-control"
                                name="add_tempat" required>
                        </div>
    ...
    <div class="modal fade text-left" id="edit_data_seminar" tabindex="-1" role="dialog" aria-labelledby="modal_add"
        aria-hidden="true">
        <div class="modal-dialog modal-create2 modal-dialog-centered modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form>
                    <div class="modal-body">
                        <label>nama:</label>
                        <div class="form-group">
                            <input type="hidden" name="edit_seminarid" readonly>
...
@section('js_asset')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#tabel_seminar').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ url('/api/v1/seminar/list') }}",
                columns: [{
                        data: 'nama'
                    },
                    {
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            const button_tambah_peserta =
                                ...
                                full.id + "', '" + full.nama + "')\">➕</a>";
                            const button_edit =
                                "<a class=\"btn icon btn-primary mx-1 my-1\" data-toggle=\"tooltip\" data-title=\"Edit\" title=\"Edit\" href=\"javascript:void(0)\" onclick=\"edit_seminar('" +
                                full.id + "')\">⚙</a>";
                            const button_delete =
                                "<a class=\"btn icon btn-danger mx-1 my-1\" data-toggle=\"tooltip\" data-title=\"Delete\" title=\"Delete\" href=\"javascript:void(0)\" onclick=\"remove_seminar('" +
                                full.id + "')\">🚮</a>";
    ...
    <script>
        ...
        $("form").on("submit", function(e) {
            ...
            if (
                ...
                var url = "{{ url('/api/v1/seminar/add') }}";
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            ...
                            url: "{{ url('/api/v1/seminar/delete') . '/' }}" + id,
                            type: "POST",
                            //jika ajax sukses
                            success: function(data) {
                                swal.fire({
        ...
        function add_kategori_option() {
            ...
            $.ajax({
                ...
                url: "{{ url('/api/v1/kategori/seminar/list') }}",
                type: "GET",
                async: false,
                dataType: 'json',
                //jika ajax sukses
        ...
        function add_pengajar_option(id_seminar) {
            var token = $('meta[name="csrf-token"]').attr('content');
            ...
            $.ajax({
                ...
                url: "{{ url('/api/v1/user/pengajar/list/') . '/' }}" + id_seminar,
                type: "GET",
                async: false,
                dataType: 'json',
                //jika ajax sukses
        ...
        function add_pelajar_option(id_seminar) {
            var token = $('meta[name="csrf-token"]').attr('content');
            $('#add_peserta_pelajar')
                .find('option')
                .remove()
            ...
            $.ajax({
                ...
                url: "{{ url('/api/v1/user/pelajar/list/') . '/' }}" + id_seminar,
                type: "GET",
                async: false,
                dataType: 'json',
                //jika ajax sukses
        ...
        function edit_seminar(id) {
            ...
            $.ajax({
                ...
                url: "{{ url('/api/v1/seminar/list/') . '/' }}" + id,
                type: "GET",
                async: false,
                dataType: 'json',
                //jika ajax sukses
                success: function(data) {
                    ...
                    $('input[name="edit_seminarid"]').val(data.id);
                    $('input[name="edit_nama"]').val(data.nama);
                    $('input[name="edit_video"]').val(data.video);
                    $('input[name="edit_tempat"]').val(data.tempat);
        ...
        function reset_form() {
            ...
            $('#add_peserta_pengajar').multiselect('destroy');
            $('#add_peserta_pelajar').multiselect('destroy');
            $('#tabel_seminar').DataTable().ajax.reload();
            $('#add_data_seminar').modal('hide');
            $('#add_data_peserta').modal('hide');
            $('#edit_data_seminar').modal('hide');
        }
    </script>
